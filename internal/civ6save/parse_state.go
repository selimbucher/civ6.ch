...
func ParseState(data []byte) (*GameState, error) {
	...
	gs := &GameState{}
	gs.Religions = make([]ReligionState, 0, religionCount)
	for i := 0; i < religionCount; i++ {
		...
		gs.Religions = append(gs.Religions, ReligionState{
	...
	for iPlayer := 0; iPlayer < playerCount; iPlayer++ {
		...
		for iCity := 0; iCity < cityCount; iCity++ {
			city := &ps.Cities[iCity]
			city.ID = int(r.readU32())
			if err := r.assertU32(0x33); err != nil {
				return nil, errors.New("expected 0x33 in city")
			}
			r.skip(4)
			r.skip(4)
			...
			for i := 0; i < 13; i++ {
				if err := r.assertU32(6); err != nil {
					return nil, errors.New("expected 6 in city readMap loop")
				}
				r.readMapDiscard()
			}
			...
			for i := 0; i < count; i++ {
				if err := r.assertU32(0x33); err != nil {
					return nil, errors.New("expected 0x33 in city district sub-loop")
				}
				r.skip(4)
				r.skip(4)
				r.skip(8)
			...
			r.skip(17)
			if err := r.assertU32(6); err != nil {
				return nil, errors.New("expected 6 before city name readMap")
			}
			r.readMapDiscard()
			r.skip(13)
			...
			for i := 0; i < count; i++ {
				if err := r.assertU32(0x2C0F4A46); err != nil {
					return nil, errors.New("expected 0x2C0F4A46 in production")
				}
				...
				r.skip(4)
				if prodType > 4 {
					return nil, errors.New("unexpected prodType")
				}
			...
			count = int(r.peek32())
			if count != 64 {
				return nil, errors.New("expected 64 in city player bitmap")
			}
			r.skip(4)
			r.skip(64)
			r.skip(4)
			...
			for i := 0; i < 14; i++ {
				if err := r.assertU32(6); err != nil {
					return nil, errors.New("expected 6 in 14-map loop")
				}
				r.readMapDiscard()
			}
			r.skip(4) // 0e
		...
		for i := 0; i < districtCount; i++ {
			...
			if err := r.assertU32(0x0f); err != nil {
				return nil, errors.New("expected 0x0f in district")
			}
			r.skip(4)
			...
			for k := 0; k < 3; k++ {
				if err := r.assertU32(6); err != nil {
					return nil, errors.New("expected 6 in district map loop")
				}
				r.readMapDiscard()
			}
			r.skip(12)
			if err := r.assertU32(6); err != nil {
				return nil, errors.New("expected 6 in district map 4")
			}
			r.readMapDiscard()
			if err := r.assertU32(6); err != nil {
				return nil, errors.New("expected 6 in district map 5")
			}
			r.readMapDiscard()
			r.skip(4)
			count2 := int(r.readU32())
		...
		for iUnit := 0; iUnit < unitCount; iUnit++ {
			...
			r.skip(54)
			if err := r.assertU32(6); err != nil {
				return nil, errors.New("expected 6 in unit map")
			}
			r.readMapDiscard() // 06
			count = int(r.readU32())
			r.skip(count * 4)
		...
		// This is more reliable than parsing the variable great-person class loops
		{
			...
			for off := 0; off < 2000; off++ {
				if r.pos+off+45 > len(data) {
					break
				}
				if binary.LittleEndian.Uint32(data[r.pos+off:]) == 0x16 &&
					binary.LittleEndian.Uint32(data[r.pos+off+41:]) == 64 {
					r.pos = r.pos + off + 41
					found = true
					break
				}
		...
		// ── City yields (inside player loop) ─────────────────────────────────
		cityYieldCount := int(r.readU32())
		for i := 0; i < cityYieldCount; i++ {
			cityID := int(r.readU32())
			if err := r.assertU32(0x11); err != nil {
				return nil, fmt.Errorf("player %d city %d: expected 0x11 at pos %d, got %08x", trueiPlayer, i, r.pos, r.peek32())
			}
			r.skip(4 + 4 + 4 + 4 + 4 + 4) // skip 0x11 + 5 more
			if err := r.assertU32(6); err != nil {
				return nil, fmt.Errorf("player %d city %d: expected 6 before yields at pos %d, got %d", trueiPlayer, i, r.pos, r.peek32())
			}
			yields := r.readMapFloat()
			if err := r.assertU32(6); err != nil {
				return nil, fmt.Errorf("player %d city %d: expected 6 before yields2 at pos %d, got %d", trueiPlayer, i, r.pos, r.peek32())
			}
			...
			for j := 0; j < count2; j++ {
				if err := r.assertU32(0x04); err != nil {
					return nil, errors.New("expected 0x04 in yield sub-loop")
				}
				r.skip(4 + 4)
				if err := r.assertU32(0x64); err != nil {
					return nil, errors.New("expected 0x64 in yield sub-loop")
				}
				r.skip(4 + 35)
				if err := r.assertU32(0x40); err != nil {
					return nil, errors.New("expected 0x40 in yield sub-loop")
				}
				count3 := int(r.readU32())
				r.skip(count3 * 4)
				r.skip(12)
			...
			for j := 0; j < count2; j++ {
				if err := r.assertU32(1); err != nil {
					return nil, errors.New("expected 1 in c2*69 loop")
				}
				r.skip(4 + 4 + 61)
			}
			// c2 * 3maps
		...
		idxIA := bytes.Index(data[r.pos:], endIAStuff)
		if idxIA == -1 {
		...
		r.skip(1)
		if count2Raw != 0 {
			idxPS := bytes.Index(data[r.pos:], endPlayerStuff)
			if idxPS == -1 {
...
// floatBits converts float32 bits back to float
func floatBits(v uint32) float32 {
